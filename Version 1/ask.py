...
def retrieve_hybrid(chunks, emb, query, top_k=8, alpha=0.5):
    # BM25
...
def main(chunks_jsonl, embeddings_npy, question, model_name="qwen2.5:7b-instruct"):
    chunks = load_chunks(chunks_jsonl)
    emb = np.load(embeddings_npy)

    sources = retrieve_hybrid(chunks, emb, question, top_k=8, alpha=0.5)
