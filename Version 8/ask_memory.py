...
def main():
    ...
    # - create_agent runs on LangGraph under the hood and can persist graph state between calls.
    # - The checkpointer is the component that saves/loads that state (checkpoints).
    # - InMemorySaver stores checkpoints in RAM (NOT persistent), keyed by a thread_id we pass at invoke time.
    # - On each invoke, the runtime:
    #     1) loads the latest state for thread_id (including prior messages),
    #     2) appends the new user message,
    #     3) runs the agent loop,
    #     4) saves the updated state back under the same thread_id.
    # - Result: follow-up questions work while the process is running. Restarting the script resets memory.
    
    # Middleware stack:
    # 1) ContextEditingMiddleware + ClearToolUsesEdit:
    #    - When the history approaches 8000 tokens, it prunes tool calls + tool results from the stored message history.
    #    - It keeps only the most recent 4 tool uses.
    #
    # 2) SummarizationMiddleware:
    #    - After tool pruning, if the thread is still large 8000 tokens, it replaces
    #      the older part of the conversation with a compact summary while preserving
