"""
Evaluation script for hybrid retrieval with cross-encoder reranking.
Tests on easy.jsonl and hard.jsonl test files.
Uses the retrieval strategy from ask_rerank.py.
"""
...
def retrieve_hybrid_candidates(chunks, chunk_ids, emb, bm25, embed_model, query, top_k_recall=40, alpha=0.5):
    """
    First stage: Hybrid retrieval combining BM25 and embeddings.
    Returns list of (chunk, chunk_id, score) tuples.
    """
    # BM25 scores
    ...
    ranked_indices = np.argsort(-final)[:top_k_recall]

    candidates = [(chunks[i], chunk_ids[i], final[i]) for i in ranked_indices]
    return candidates


def rerank_cross_encoder(reranker, query, candidates, top_k_final=10):
    """
    Second stage: Rerank candidates using cross-encoder.
    ...
    reranked = [(candidates[i][1], scores[i]) for i in order]
    return reranked


def retrieve_with_rerank(chunks, chunk_ids, emb, bm25, embed_model, reranker, query,
                         top_k_recall=40, top_k_final=10, alpha=0.5):
    """
    Full two-stage retrieval: hybrid recall + cross-encoder reranking.
    Returns list of (chunk_id, score) tuples.
    """
    # Stage 1: Get candidates
    candidates = retrieve_hybrid_candidates(
        chunks, chunk_ids, emb, bm25, embed_model, query,
        top_k_recall=top_k_recall, alpha=alpha
    )

    # Stage 2: Rerank with cross-encoder
    results = rerank_cross_encoder(reranker, query, candidates, top_k_final=top_k_final)

    return results


def evaluate_retrieval(test_data, chunks, chunk_ids, emb, bm25, embed_model, reranker,
                       top_k_values=[1, 3, 5, 10], top_k_recall=40, alpha=0.5):
    """
    ...
    for test in test_data:
        ...
        results = retrieve_with_rerank(
            chunks, chunk_ids, emb, bm25, embed_model, reranker, question,
            top_k_recall=top_k_recall, top_k_final=max_k, alpha=alpha
        )
...
def main():
    ...
    print("=" * 60)
    print("Hybrid Retrieval with Cross-Encoder Reranking Evaluation")
    print("=" * 60)
    ...
    # Evaluate with alpha = 0.5, top_k_recall = 40 (as in ask_rerank.py)
    top_k_values = [1, 3, 5, 10]
    alpha = 0.5
    top_k_recall = 40

    print(f"\nAlpha = {alpha} (BM25={alpha:.0%}, Embedding={1-alpha:.0%})")
    print(f"Top-K Recall = {top_k_recall}")
    print("=" * 60)

    # Evaluate on easy
    print("\n[EASY TEST SET]")
    easy_metrics = evaluate_retrieval(
        easy_tests, chunks, chunk_ids, emb, bm25, embed_model, reranker,
        top_k_values, top_k_recall, alpha
    )
    ...
    hard_metrics = evaluate_retrieval(
        hard_tests, chunks, chunk_ids, emb, bm25, embed_model, reranker,
        top_k_values, top_k_recall, alpha
    )
