...
def retrieve_hybrid_candidates(chunks, bm25, emb, embed_model, query, top_k_recall=40, alpha=0.5):
...
def rerank_cross_encoder(reranker, query, candidates, top_k_final=8):
    ...
    return [candidates[i] for i in order], scores
...
if st.button("Ask"):
    ...
    candidates = retrieve_hybrid_candidates(chunks, bm25, emb, embed_model, question, top_k_recall=TOP_K_RECALL, alpha=ALPHA)
    sources, _ = rerank_cross_encoder(reranker, question, candidates, top_k_final=TOP_K_FINAL)
